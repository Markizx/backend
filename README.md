...
3. **Сервисы** (`/services`) - содержат бизнес-логику:
   ...
   - `aws.service.ts` - хранение файлов в AWS S3
   - `mailService.ts` - отправка электронных писем
4. **Обертки для внешних API** (`/services/ai`) - изолированное взаимодействие с API AI-сервисов:
   - `openai.service.ts` - OpenAI API (GPT-4, DALL-E, модели Vision)
   - `stability.service.ts` - Stability AI API (SD3.5, inpainting, outpainting)
   - `runway.service.ts` - Runway ML API (генерация видео)
   - `grok.service.ts` - Grok AI (x.ai) API (чат-модели и генерация изображений)
...
- **Authentication**: JWT, Passport (Google OAuth, Apple Sign In)
- **Payments**: Stripe
- **Content Generation**: 
